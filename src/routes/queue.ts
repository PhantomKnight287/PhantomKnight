...
router.get("/:guildid", async (req, res) => {
  try {
    ...
    if (!isPlaying) {
      return res.status(200).send({ message: "No Music is Currently Playing" });
    }
    try {
      const currentlyPlaying = player.getQueue(req.params.guildid).current;
...
router.post("/:guildId/user", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ message: { inVc: channelId[0] == channelId[1] ? true : false } });
  } catch (error) {
